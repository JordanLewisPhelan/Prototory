...
void PerlinNoiseGenerator::initializePermutation(uint32_t t_seed) const
{
    ...
    for (int i = 0; i < PERM_SIZE; ++i)
    {
        ...
        m_permutation[PERM_SIZE + i] = l_permTable[i]; 
    }
...
float PerlinNoiseGenerator::noise(float t_x, float t_y) const
{
    ...
    int X = static_cast<int>(std::floor(t_x)) & PERM_MASK; 
    ...
    float lerpTop = lerp(l_smoothX, l_gradientTopLeft, l_gradientTopRight);    
    float lerpBottom = lerp(l_smoothX, l_gradientBottomLeft, l_gradientBottomRight); 
...
float PerlinNoiseGenerator::gradient(int t_hash, float t_x, float t_y) const
{
    ...
    int l_hash = t_hash & 15; 

    
    
    float u = (l_hash < 8) ? t_x : t_y;  
